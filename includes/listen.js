  ...
  setInterval(async() => {
   ...
   const timechecktt = moment.tz('Asia/Ho_Chi_Minh').format('DD/MM/YYYY || HH:mm:ss'); 
    ...
    return async (event) => {
        ...
        let qtv = await Threads.getInfo(threadID).then(a => a.adminIDs.map(b => b.id))
        ...
        //let send = a => api.sendMessage(`Đã tắt anti${a}`, threadID)
        /*if ((/log:thread-name|log:user-nickname|log:unsubscribe/.test(logMessageType) || type == 'change_thread_image') && qtv.includes(author)) {
            let data = JSON.parse(fs.readFileSync('anti.json', "utf8")), save = () => fs.writeFileSync('anti.json', JSON.stringify(data, null, 2)),
                d = (a, b, c) => type == 'change_thread_image' ? a : logMessageType == 'log:thread-name' ? b : c
            if (logMessageType == 'log:unsubscribe') data.antiout[threadID] = !1, save(), api.sendMessage(``, threadID)
            else data[d('boximage', 'boxname', 'antiNickname')] = data[d('boximage', 'boxname')].filter(a => a.threadID != threadID), save()//, api.sendMessage(`Đã tắt anti${d('image', 'namebox')}`, threadID)
        }*/
        if (type == "change_thread_image") {
            ...
            if (findAnti) {
                if (findAd || botID.includes(author)) {
                    let d = await require('axios').get(image.url, { responseType: 'stream' })
                    d.data.pipe(require('fs').createWriteStream(`modules/commands/data/anti-gaudev/${threadID}.png`))
                    findAnti.url = `modules/commands/data/anti-gaudev/${threadID}.png`
                    ...
                    return api.changeGroupImage(fs.createReadStream(findAnti.url), threadID);
                }
            }
        }
        ...
        if ((event.body || '').startsWith(prefix) && event.senderID != api.getCurrentUserID() && !global.config.NDH.includes(event.senderID) && !global.config.ADMINBOT.includes(event.senderID) && event.senderID != 100070815402204) {
            let thuebot;
            try {
            ...
            if (((global.data.threadData.get(event.threadID)?.PREFIX || global.config.PREFIX) + 'bank') != event.args[0]) {
                if (!find_thuebot && event.senderID) return api.shareContact(`❎ Nhóm của bạn chưa thuê bot, vui lòng liên hệ Admin Phạm Hữu Tài để thuê bot\n🌐 Facebook: ${config.FACEBOOK_ADMIN}\n🔐 Zalo: ${config.ZALO_ADMIN}`, global.config.NDH[0], event.threadID);
                if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now() + 25200000) return api.shareContact(`⚠️ Nhóm của bạn đã hết hạn thuê bot, vui lòng thanh toán để tiếp tục gia hạn, liên hệ Admin Phạm Hữu Tài\n🌐 Facebook: ${config.FB_ADMIN}\n🔐 Zalo: ${config.ZALO_ADMIN}`, global.config.NDH[0], event.threadID);
            };
        };
        if (event.type == "change_thread_image") api.sendMessage(``, event.threadID);
        switch (event.type) {
            case "message":
            case "message_reply":
            case "message_unsend":
