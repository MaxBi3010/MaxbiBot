class SlotGame {
  ...
  async run({
    ...
    if (incl(fruits, fruit)) {
      ...
      // Giảm tỷ lệ trúng còn 70%
      let isWin = Math.random() <= 0.7; // 70% xác suất trúng
      if (!isWin) {
        // Nếu thua, đảm bảo không có fruit nào khớp
        result1 = result2 = result3 = random2(fruits.filter((r) => r !== fruit));
      }
